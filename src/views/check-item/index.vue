  ...
  <div>
    <el-row :gutter="10">
      ...
      <el-col :span="19">
        ...
        <el-button type="primary" icon="el-icon-circle-plus-outline" plain @click="add()">添加检查项目</el-button>
...
<script>
  import CheckItemEdit from '@/views/check-item/edit'
  ...
  export default {
    ...
    methods: {
      getData() {
        this.axios.get('/check-item/list', response => {
      ...
      handleDelete(id) {
        this.axios.del('/check-item/del', response => {
          this.getData()
        }, {
          id: id
        })
      ...
      batchdel() {
        if (this.selectedrow.length === 0) {
          ...
          this.axios.del('/check-item/batchdel', response => {
            this.getData()
          }, {
            ids: ids
          })
      ...
      handleBack(id) {
        this.axios.del('/check-item/back', response => {
          this.getData()
        }, {
          id: id
        }, 'back')
