<template>
  <div>
    <el-row :gutter="10">
      <el-col :span="5">
        <el-input v-model="search.name" placeholder="请输入要查的部门名称"></el-input>
      </el-col>
      <el-col :span="19">
    ...
    <el-table :data="result.tableData" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="办公地址" prop="address">
      </el-table-column>
      <el-table-column label="负责人" prop="leader">
      </el-table-column>
     <el-table-column label="部门类型" >
       <template slot-scope="scope">
         <span v-if="scope.row.type === 1">医疗科室</span>
         <span v-else>非医疗科室</span>
       </template>
     </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(id) {
        this.axios.del('/department/del', response => {
          this.getData()
        }, {
          id: id
        })
      ...
      batchdel() {
        if (this.selectedrow.length === 0) {
          ...
          this.axios.del('/department/batchdel', response => {
            this.getData()
          }, {
            ids: ids
          })
      ...
      handleBack(id) {
        this.axios.del('/department/back', response => {
          this.getData()
        }, {
          id: id
        }, 'back')
