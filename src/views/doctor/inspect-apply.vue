...
<!-- @selection-change="handleSelectionChange" -->
        ...
        <el-dialog title="选择检验项目" :visible.sync="show" :close-on-click-modal="false" width="800px">
          <SelectInspect v-if="show" :show.sync="show"  @selectInspectItem="selectInspectItem" ></SelectInspect>
        </el-dialog>
...
<script>
  import SelectInspect from '@/views/doctor/select-inspect'
  export default {
    name: 'InspectApply',
    props:[
      'register'
    ...
    methods: {
      ...
      selectInspectItem(inspectItem) {
         let exists = false;
         for(let i = 0 ;i <this.tableData.length ;i++){
           if(this.tableData[i].itemId == inspectItem.id){
             exists = true;
           }
         }
         if(!exists){
      ...
      save() {
        ...
        this.axios.post('/inspect-apply/save',response =>{
            this.getData()
        },{
      ...
      getData() {
        //查询当前患者已申请的检查项目
        this.axios.post('/inspect-apply/list',resposne =>{
