<template>
  <div>
    <el-row :gutter="10">
      <el-col :span="5">
        <el-input v-model="search.name" placeholder="请输入要查的医生级别"></el-input>
      </el-col>
      <el-col :span="19">
    ...
    <el-table :data="result.tableData" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="挂号级别" prop="name">
      </el-table-column>
      <el-table-column label="问诊金额" prop="fee">
      </el-table-column>
...
<script>
  import RegistLevelEdit from '@/views/registlevel/edit'
  ...
  export default {
    ...
    methods: {
      ...
      handleDelete(id) {
        this.axios.del('/regist-level/del', response => {
          this.getData()
        }, {
          id: id
        })
      ...
      batchdel() {
        if (this.selectedrow.length === 0) {
          ...
          this.axios.del('/regist-level/batchdel', response => {
            this.getData()
          }, {
            ids: ids
          })
      ...
      handleBack(id) {
        this.axios.del('/regist-level/back', response => {
          this.getData()
        }, {
          id: id
        }, 'back')
